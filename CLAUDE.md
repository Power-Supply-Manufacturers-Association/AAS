...
- `schemas/AAS.json` — top wrapper `{ inputs, <family>, outputs }` with `anyOf` (bare component | full doc) + a `oneOf` over the 14 family discriminators. Every valid AAS document is also a valid PEAS document (the `analog` branch).
